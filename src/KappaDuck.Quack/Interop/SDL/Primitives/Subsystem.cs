// Copyright (c) KappaDuck.
// Licensed under the MIT license.

namespace KappaDuck.Quack.Interop.SDL.Primitives;

[Flags]
internal enum Subsystem : uint
{
    /// <summary>
    /// No subsystem.
    /// </summary>
    None = 0x00000000,

    /// <summary>
    /// Enable the audio subsystem. Implicitly enables the <see cref="Events"/> subsystem.
    /// </summary>
    Audio = 0x00000010u,

    /// <summary>
    /// Enable the video subsystem. Implicitly enables the <see cref="Events"/> subsystem.
    /// </summary>
    /// <remarks>
    /// It should be enabled on the main thread.
    /// </remarks>
    Video = 0x00000020u,

    /// <summary>
    /// Enable the joystick subsystem. Implicitly enables the <see cref="Events"/> subsystem.
    /// </summary>
    /// <remarks>
    /// It should be enabled on the same thread as <see cref="Video"/> subsystem.
    /// </remarks>
    Joystick = 0x00000200u,

    /// <summary>
    /// Enable the Force Feedback subsystem.
    /// </summary>
    Haptic = 0x00001000u,

    /// <summary>
    /// Enable the gamepad subsystem. Implicitly enables the <see cref="Joystick"/> subsystem.
    /// </summary>
    Gamepad = 0x00002000u,

    /// <summary>
    /// Enable the Events subsystem.
    /// </summary>
    Events = 0x00004000u,

    /// <summary>
    /// Enable the sensor subsystem. Implicitly enables the <see cref="Events"/> subsystem.
    /// </summary>
    Sensor = 0x00008000u,

    /// <summary>
    /// Enable the camera subsystem. Implicitly enables the <see cref="Events"/> subsystem.
    /// </summary>
    Camera = 0x00010000u,

    /// <summary>
    /// The SDL_ttf extension library.
    /// </summary>
    /// <remarks>
    /// This is not an SDL subsystem; it is an extension to be more simpler to use with the engine
    /// by initializing the library through SDL_ttf rather than <see cref="SDL3.InitSubSystem(Subsystem)"/>.
    /// </remarks>
    TTF = 0x40000000,

    /// <summary>
    /// The SDL_mixer extension library.
    /// </summary>
    /// <remarks>
    /// This is not an SDL subsystem; it is an extension to be more simpler to use with the engine
    /// by initializing the library through SDL_mixer rather than <see cref="SDL3.InitSubSystem(Subsystem)"/>.
    /// </remarks>
    Mixer = 0x20000000

}
