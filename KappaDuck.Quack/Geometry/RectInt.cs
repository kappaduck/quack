...
public struct RectInt(int x, int y, int width, int height) : IEquatable<RectInt>, ISpanFormattable, IUtf8SpanFormattable
{
    /// <summary>
    ...
    /// </summary>
    public readonly Vector2Int Center => new(X + (Width / 2), Y + (Height / 2));
    ...
    public readonly bool ContainsAll(ReadOnlySpan<Vector2Int> points)
    {
    ...
    /// <param name="points">The points to check.</param>
    /// <returns><see langword="true"/> if all points are within the rectangle; otherwise, <see langword="false"/>.</returns>
    public readonly bool ContainsAll(IEnumerable<Vector2Int> points)
    {
        if (IsEmpty)
            return false;
        ...
        {
            Vector2Int[] array => ContainsAll(array),
            List<Vector2Int> list => ContainsAll(CollectionsMarshal.AsSpan(list)),
    ...
    public readonly bool ContainsAny(ReadOnlySpan<Vector2Int> points)
    {
        if (IsEmpty || points.IsEmpty)
            return false;
    ...
    /// <param name="points">The points to check.</param>
    /// <returns><see langword="true"/> if any point is within the rectangle; otherwise, <see langword="false"/>.</returns>
    public readonly bool ContainsAny(IEnumerable<Vector2Int> points)
    {
        if (IsEmpty)
            return false;
        ...
        return points switch
        {
            Vector2Int[] array => ContainsAny(array),
            List<Vector2Int> list => ContainsAny(CollectionsMarshal.AsSpan(list)),
    ...
    /// </remarks>
    /// <returns>An array of points within the rectangle.</returns>
    public readonly Vector2Int[] ToPoints()
    {
        if (IsEmpty)
            return [];

        Vector2Int[] points = new Vector2Int[Area];
        int index = 0;
    ...
    public static RectInt Intersect(RectInt left, RectInt right)
    {
    ...
    /// <inheritdoc/>
    public override readonly bool Equals([NotNullWhen(true)] object? obj) => obj is RectInt other && Equals(other);
