...
/// <summary>
/// Represents the progress bar for the window's taskbar icon.
/// </summary>
public sealed class TaskbarProgress
{
    ...
    /// </remarks>
    /// <exception cref="QuackInteropException">Thrown when failing to reset the window's progress state or value.</exception>
    public void Reset()
    {
        _isCompleted = false;
        _isReporting = false;
        ...
        {
            QuackInteropException.ThrowIfFailed(SDL3.Windows.SetWindowProgressState(handle, ProgressState.None));
            QuackInteropException.ThrowIfFailed(SDL3.Windows.SetWindowProgressValue(handle, 0));
        });
    }
    ...
    public void Start(Action<Progress> action, int total = 100)
    {
        ...
        StartReporting(ProgressState.Normal);
    ...
    {
        QuackException.ThrowIf(_isReporting, "Cannot begin a new progress report while another is in progress.");
        StartReporting(ProgressState.Indeterminate);
    ...
    private void Fail(Exception ex)
    {
        ...
        _window.Invoke(handle => SDL3.Windows.SetWindowProgressState(handle, ProgressState.Error));
    }
    ...
    internal void Report(float value)
    {
        ...
        if (MathF.ApproximatelyZero(progress - _oldValue))
            return;
    ...
    private void StartReporting(ProgressState state)
    {
        _isCompleted = false;
        _isReporting = true;

        _window.Invoke(handle => SDL3.Windows.SetWindowProgressState(handle, state));
    }

    private void SetValue(float value)
    {
        TimeSpan now = QuackEngine.ElapsedTime;

        if (MathF.ApproximatelyZero(value - _oldValue) && now.TotalMilliseconds - _lastAppliedMilliseconds < ThrottleInMilliseconds)
            return;

        _oldValue = value;
        _lastAppliedMilliseconds = now.TotalMilliseconds;

        _window.Invoke(handle => SDL3.Windows.SetWindowProgressValue(handle, value));
    }
}
